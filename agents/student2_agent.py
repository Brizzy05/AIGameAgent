...
class Student2Agent(Agent):
    ...
    def step(self, chess_board, my_pos, adv_pos, max_step):
        ...
        move = self.minimax(True, my_pos, adv_pos, chess_board, board_size, 2, 0)
    ...
    def minimax(self, isMaximizing, my_pos, adv_pos, chess_board, board_size, depth, direction):
        max_step = (board_size + 1) // 2
        move_list = self.valid_move(chess_board, my_pos, max_step, board_size, adv_pos)

        #print(len(move_list))
        #print("History", "My_pos", my_pos, "direction", "Opponent", adv_pos, "Turn", isMaximizing, "depth", depth)

        if depth == 0:
            move_listb = self.valid_move(chess_board, adv_pos, max_step, board_size, my_pos)
            count = self.heuristic(chess_board, move_list, move_listb, my_pos, adv_pos, isMaximizing, direction)
        ...
        for mv in move_list:
            ...
            if isMaximizing:
                sim_score = self.minimax(False, my_pos=adv_pos, adv_pos=new_pos, chess_board=chess_board,
                                         board_size=board_size, depth=depth - 1, direction=d)
            else:
        ...
        #print("End MinMax", best, isMaximizing, "depth", depth)

        return best

    @staticmethod
    def heuristic(chess_board, move_lista, move_listb, my_pos, adv_pos, isMax, direction):
        ...
        # check how walls surround enemy
        if isMax:
