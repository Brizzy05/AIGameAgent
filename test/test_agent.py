...
def test_step(board_size, agent):
    ...
    world = World(player_1=agent,player_2="random_agent",board_size=board_size,display_ui=False)
    assert world.turn == 0
    cur_player, cur_pos, adv_pos = world.get_current_player()
    next_pos, dir = cur_player.step(deepcopy(world.chess_board),tuple(cur_pos),tuple(adv_pos),world.max_step)
